...
public class PentahoModel extends PentahoService {
	...
	public void genereMetaData() {
		...
		try {
			URL url = new URL("http://localhost:8080/pentaho/AdhocWebService?userid=" + userName + "&password=" + password + "&model=" + modelName
					+ "&component=getbusinessmodel&domain=" + domain + "/metadata.xmi");
			...
			for (int s = 0; s < viewNodeLst.getLength(); s++) {
				...
				for (int t = 0; t < columnNodeList.getLength(); t++) {
					...
					if (columnNode.getNodeName().equals("column")) {
						...
						// on ajoute la catégorie pour chaque Business Column
						// (ça pourra servir...)
	...
	public String getSelectionFilter() {
		...
		for (int i = 0; i < pentahoResultSetColumnNamesMetaData.length; i++) {
			selectionFilter = selectionFilter + "<selection><view>" + this.getPentahoResultSetViewNamesMetaData()[i] + "</view><column>"
					+ this.getPentahoResultSetColumnNamesMetaData()[i] + "</column></selection>";
		}
