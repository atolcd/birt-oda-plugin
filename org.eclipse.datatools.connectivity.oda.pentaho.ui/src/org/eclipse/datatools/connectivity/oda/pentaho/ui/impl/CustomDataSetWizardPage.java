...
package org.eclipse.datatools.connectivity.oda.pentaho.ui.impl;
...
import org.eclipse.datatools.connectivity.oda.pentaho.impl.PentahoModel;
import org.eclipse.datatools.connectivity.oda.pentaho.impl.PentahoService;
...
public class CustomDataSetWizardPage extends DataSetWizardPage
{
	...
	private transient List businessColumnsList;
	private transient Combo modelListCombo;

	private transient PentahoService pentahoService;
	private transient PentahoModel pentahoModel;

	String[][] testListe = new String[3][4];
     ...
     * @throws OdaException 
     */
    private Control createPageControl( Composite parent ) throws OdaException
    {
		initPentahoModels();
		...
		// ***********************
		String modele = createModelListCombo(composite);
		String listeColonnes = createBusinessColumnsList(composite, modele);

		// ***********************

		m_queryTextField = new Text(composite, SWT.Hide | SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL);
        GridData data = new GridData( GridData.FILL_HORIZONTAL );
        data.heightHint = 100;
		m_queryTextField.setVisible(false);
	...
	private String createModelListCombo(Composite parent) throws OdaException {

		GridData gridData = new GridData(GridData.FILL_HORIZONTAL);
		Label label = new Label(parent, SWT.LEFT);
		label.setText("Sélectionner un modèle métier :");

		gridData = new GridData(GridData.FILL_HORIZONTAL);
		gridData.minimumWidth = 150;
		modelListCombo = new Combo(parent, SWT.DROP_DOWN | SWT.READ_ONLY);

		for (int i = 0; i < modelLabels.length; i++) {
			modelListCombo.add(modelLabels[i]);
		}

		modelListCombo.setLayoutData(gridData);

		if (modelLabels.length > 0) {
			int selectedIndex = 0;
			modelListCombo.select(selectedIndex);
		}

		modelListCombo.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				int indexModelListCombo = modelListCombo.getSelectionIndex();
				updateBusinessColumnsList(indexModelListCombo);
				m_queryTextField.setText("");
			}
		});

		if (modelListCombo.getItemCount() != 0) {
			return modelListCombo.getItem(0);
		} else {
			return null;
		}
	...
	private String createBusinessColumnsList(Composite parent, final String modelLabel) {

		GridData gridData = new GridData(GridData.FILL_HORIZONTAL);
		Label label = new Label(parent, SWT.LEFT);
		label.setText("Sélectionner les colonnes métiers :");

		gridData = new GridData(GridData.FILL_HORIZONTAL);
		gridData.minimumWidth = 150;
		gridData.minimumHeight = 550;

		businessColumnsList = new List(parent, SWT.BORDER | SWT.MULTI | SWT.V_SCROLL);
		businessColumnsList.setLayoutData(gridData);

		if (modelLabel != null) {
			updateBusinessColumnsList(0);
		}

		businessColumnsList.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				int[] selection = businessColumnsList.getSelectionIndices();
				String concat = "";
				for (int i = 0; i < selection.length; i++) {
					if (i == 0) {
						concat = concat + selection[i];
					} else {
						concat = concat + ";" + selection[i];
					}
				}
				concat = modelNames[modelListCombo.getSelectionIndex()] + "#" + concat;
				m_queryTextField.setText(concat);
			}
		});
		if (businessColumnsList.getItemCount() != 0) {
			return businessColumnsList.getItem(0);
		} else {
			return null;
		}
	}

	private void updateBusinessColumnsList(int indexModelListCombo) {

		String table[] = (String[]) map.get(indexModelListCombo);
		businessColumnsList.setItems(table);
	}

	private void initPentahoModels() throws OdaException {

		if (pentahoModel != null) {
			return;
		}
		...
		if (getInitializationDesign() != null) {
			...
			String servername = connProps.getProperty("servername");
			String username = connProps.getProperty("username");
			String password = connProps.getProperty("password");
			String domain = connProps.getProperty("domain");
			System.out.println(servername + " " + username + " " + password);

			// Initialise un objet "Pentaho Model"
			this.pentahoService = new PentahoService(servername, username, password, domain);

			// Initialise les Business Models (tableau listModels)
			String[][] listeModels = pentahoService.getAllModels();

			modelNames = new String[listeModels.length];
			modelLabels = new String[listeModels.length];

			for (int i = 0; i < listeModels.length; i++) {
				modelLabels[i] = listeModels[i][1];
				modelNames[i] = listeModels[i][0];
			}

			// Affectation des colonnes métiers pour chaque modèle métier

			for (int i = 0; i < listeModels.length; i++) {
				PentahoModel pentahoModel = pentahoService.getModel(modelNames[i]);
				pentahoModel.genereMetaData();
				map.put(i, pentahoModel.getColumnLabels());
			}
		}
	...
	private void savePage( DataSetDesign dataSetDesign )
	{
        ...
        try
        {
             ...
             */
            IDriver customDriver = new org.eclipse.datatools.connectivity.oda.pentaho.impl.Driver();
